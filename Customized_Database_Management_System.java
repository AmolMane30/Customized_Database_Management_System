//-----------------------------------------------------------------------------------------------------------------------------------//
//        Project Title: Customized Database Management System
//
...
//                       -> Database can be stored in local drive and can be retrieved back whenever needed further.
//
//        Author       : Amol R. Mane
//        Date         : 11 August 2025
//-----------------------------------------------------------------------------------------------------------------------------------//
...
class CustomizedDBMS
{
    ...
    // Constructor initializes the DBMS and loads data from file if present.
    ...
    // Loads the employee database from "Employee.txt" file using serialization.
    public void LoadFromFile()
    {
        try(ObjectInputStream ois = new ObjectInputStream(new FileInputStream("Employee.txt")))
        {
            lobj = (LinkedList<Employee>) ois.readObject();
            if(!lobj.isEmpty())
            {
                Employee.Counter = lobj.getLast().Eid+1;    // Keeps ID consistent
            }
            System.out.println("Database loaded successfully from file.");
        }
    ...
    // Saves the employee database to "Employee.txt" file using serialization.
    public void SaveToFile()
    {
        try(ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("Employee.txt")))
        {
            oos.writeObject(lobj);
...
//                                               [ MAIN FUNCTION ]
//----------------------------------------------------------------------------------------------------------------------------------//
// Main Class for the Customized DBMS
        ...
        while(true)
        {
            try
            {
                ...
                else if(iOption == 13)
                {
                    ...
                    {
                        System.out.println("No database file found to delete");
                    }
                    Employee.Counter = 1;
                }
                else if(iOption == 14)
